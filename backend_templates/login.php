                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    if(mysqli_stmt_fetch($stmt)){
                        if(password_verify($password, $hashed_password)){
                            ...
                            $_SESSION["secret"] = $hashed_password;

                            //redirect user to index page
                            //header("Location: index.php");
                            echo "<script type='text/javascript'>location.href = '?page=articles';</script>";
                        } else {
                            // Password is wrong - display message
                            $password_err = "You've entered wrong password.";
...
<div class="article main_form">
    <h1 class="form_title">Login</h1>
    <p>Please fill in your credentials to login.</p>
    <form method="post">
        <div class="form-group <?php echo (!empty($username_err)) ? 'has-error' : ''; ?>">
            <label>Username</label>
            <input type="text" name="username" class="forms" value="<?php echo $username; ?>">
